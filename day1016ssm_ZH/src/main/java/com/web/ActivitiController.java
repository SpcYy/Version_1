...
public class ActivitiController {
    ...
    public void liuchengtu(String depid, String imagename, HttpServletResponse response){
        ...
        try {
            ...
            response.getWriter().print("<script>alert('文件保存在d盘，文件名是"+imagename+"');</script>");*/
    ...
    //4.删除流程部署
    @RequestMapping("/bushu/deletedepoly")
    public void deletedeploy(String depid,HttpServletResponse response){
        response.setContentType("text/html;charset=utf-8");
        ...
        try {
            ...
            try {
                response.getWriter().print("<script>alert('流程正在使用，无法删除');location.href='/bushu/getdeploys;</script>");
    ...
    public String shenpi(String taskid,ModelMap map){
        //1.通过任务id查询请假条
        LeaveBill leaveBill=activitiService.findleavebillbytaskid(taskid);
        map.put("leavebill",leaveBill);
        //2.通过任务id，查询正在活动的节点对象，并获得任务的连线
        List<String> strs =activitiService.getnames(taskid);
        map.put("names",strs);
    ...
    public String chulirenwu(String taskid,String pizhu,String result,int leaveid,HttpSession session){
        UserTb userTb = (UserTb) session.getAttribute("u1");
