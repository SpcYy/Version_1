        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <tr style="height: 25px" align="center">
                ...
                <th scope="col">
                    ...
                    <c:if test="${leave.state==3}">审核不通过</c:if>
                </th>
                <th scope="col">
                    <c:if test="${leave.state==0}">修改 删除

                    <a href="/qingjia/tijiao?leaveid=${leave.id}">提交审核</a>
                    </c:if>
                    <c:if test="${leave.state==1}">查看</c:if>
                    <c:if test="${leave.state==2}">查看 删除</c:if>
                    <c:if test="${leave.state==3}">查看 删除
                        <a href="/qingjia/tijiao?leaveid=${leave.id}">提交审核</a></c:if>
                </th>
            </tr>
            </c:forEach>
